                      ...
                      {sortedChats.map(chat => (
                        <Link key={chat.id} href={`/persona/${persona.id}?chat=${chat.id}`} className="block group" scroll={false}>
                           <div className={cn(
                            ...
                            <Button
                              variant="ghost"
                              className="h-7 w-7 shrink-0 p-0 text-muted-foreground hover:bg-transparent hover:text-destructive focus-visible:ring-0 focus-visible:ring-offset-0"
                              onClick={(e) => { 
                                e.preventDefault(); 
                                e.stopPropagation();
                            ...
                            {(persona.memories || []).length > 0 ? (
                                [...persona.memories].sort().map((memory, index) => (
                                    <div key={index} className="flex items-center justify-between text-sm p-3 rounded-md group bg-secondary/50 hover:bg-secondary/80">
                                        ...
                                        <Button variant="ghost" className="h-7 w-7 shrink-0 p-0 text-muted-foreground hover:bg-transparent hover:text-destructive focus-visible:ring-0 focus-visible:ring-offset-0" onClick={() => handleDeleteMemory(memory)}>
                                            <Trash2 className="h-4 w-4" />
                                        </Button>
                                    </div>
                                ))
